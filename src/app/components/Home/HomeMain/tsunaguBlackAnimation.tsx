...
import { letterShapes } from "./function/letterShapes-black"
...
const Shape = ({ shape, letterIndex, isFormed, colorPhase }: any) => {
  ...
  const getShapeDrawingProps = useCallback(() => {
    ...
    const startRgb = hexToRgb(initialFillColor) // 白
    const accentRgb = hexToRgb(accentColor) // アクセント色
    const endRgb = hexToRgb(targetShapeColor) // 最終色

    let currentFill = targetShapeColor

    if (startRgb && accentRgb && endRgb) {
      if (letterProgress < 0.3) {
        // フェーズ1: 白 -> アクセント色 (0 to 0.3)
        ...
        // フェーズ2: アクセント色をキープ (0.3 to 0.5)
        currentFill = accentColor
      } else {
        // フェーズ3: アクセント色 -> 最終色 (0.5 to 1.0)
